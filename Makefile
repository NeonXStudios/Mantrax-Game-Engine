...
INCLUDE_DIR = $(GRAPHICS_FLAGS) $(SCENEGARIN) $(MATHS_FLAGS) $(garin_core_includes) $(GARIN_UI_INCLUDES) $(PHYSICS_LIB_INCLUDES) #$(VRGARIN)

LIB_DIR = -L$(GRAPHICS_FLAGS_LIBS) -L$(MATHS_FLAGS_LIBS) $(garin_core_includes_dir)#-L$(VRGARINLIBSDIR)
SRC_DIR = src/

SOURCES = $(wildcard $(SRC_DIR)/*.cpp $(GRAPHICS_MAIN) $(GARINCOREMAINS) $(SCENEGARINMAINCPP) $(GARIN_UI_CPP) $(PHYSICS_LIB_CPP)) #$(VRGARINMAINS)
